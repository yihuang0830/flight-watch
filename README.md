...
| 触发 | 条件 | 频率 |
|---|---|---|
| 🚨 **紧急** | 任何航线跌破 `urgent_price`（120 USD） | 同一航线最多 4 小时一次 |
| ✈️ **定时报告** | 到了 `report_times` 配置的时段 | 一天 5 次 |
| 🔇 **免打扰** | 本地时间 00:00–08:00 | 不推送，但照常抓取入库 |
| 静默 | 其余情况 | —— |

`urgent_repeat: true` 表示只要低于阈值就报，不要求"比上次更便宜"。
但价格长期低于阈值时这会变成每 30 分钟一条（一天 48 条），
所以配了 `urgent_cooldown_hours: 4` 限流。

其余轮次静默是刻意的：每 30 分钟推一条会让人直接关掉通知，
那样真出好价时反而通知不到。
